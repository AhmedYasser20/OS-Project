...
void RevAndSetMsgFormProcesses()
{
    ...
    printf(" RES2 =I AM PROCESS %d ORDER = %d QUM= %d Rem Time =%d Exce Time=%d TYPE = %d \n ",temp.type,temp.Order,temp.Qutam,temp.remainingtime,temp.ExceTime,temp.type);
    ...
    {
        temp.Order = WAIT;
        PCB_Array[processID_Now].State = WAITING;
    }
    temp.type = pidnow;
        int reccc=msgsnd(QueueProcessesKey, &temp, (sizeof(temp.ExceTime) + sizeof(temp.Order) + sizeof(temp.remainingtime) + sizeof(temp.Qutam)), IPC_NOWAIT);
        if(reccc==-1)
        {
            puts("HERE");
        }
     printf(" RES3 =I AM PROCESS %d ORDER = %d QUM= %d Rem Time =%d Exce Time=%d TYPE = %d \n ",temp.type,temp.Order,temp.Qutam,temp.remainingtime,temp.ExceTime,temp.type);
}
    ...
    do
    {
        ...
        if (Rev != -1)
        {
            ...
            else if (Algo == 3)
            {
                ...
                PCB_Array[TempCount].State=READY;
                PCB_Array[TempCount].itsLocationInArray = TempCount;
                TempCount++;
            }
        }
